...
class SimpleProtocol(Protocol):
    ...
    def connectionLost(self, reason):
        ...
        for f in flieList:
            if 'tmp' in f:
                print "removing tmperary image file --------",self.imageDir + f
                os.remove(self.imageDir + f)
        videoList = os.listdir(self.videoDir)
        for v in videoList:
            if 'MODEL' in v:
                print "removing tmperary video file --------",self.videoDir + v
                os.remove(self.videoDir + v)

    def dataReceived(self, data):        
        if data == "model1":
            self.isImage = True
            now = datetime.datetime.now().strftime('%Y%m%d-%H%M')
            dirname = self.transport.client[0] + '-' + now + '/'
            self.imageDir  = self.imageDir + dirname
            ...
            if self.isImage is True:                  
                ...
                if not self.outWriter:
                    ...
                    self.outWriter = cv2.VideoWriter(videoName,fourcc, 10.0,frameSize)     
                ...
                if not self.imageCentroid:
                    ...
                    if centroidDistance > self.centroidDistanceLimit:
                        ...
                        if not self.outWriter:
                            ...
                            frameSize = (frame.shape[1],frame.shape[0])                            
                            self.outWriter = cv2.VideoWriter(videoName,fourcc, 2.0,frameSize) 
                ...
                if faceFlag is True:
                    ...
                    if str(label) in labelsDict:
                        ...
                        if peopleName in self.peopleNameDict:
                            self.peopleNameDict[peopleName] = self.peopleNameDict[peopleName] + 1
                        else:
                            self.peopleNameDict[peopleName] = 1
                        numCount = 0
                        for key in self.peopleNameDict:
                            numCount = numCount + self.peopleNameDict[key]
                        if numCount == 3:
                            keys = self.peopleNameDict.keys()
                            values = self.peopleNameDict.values()
                            maxIndex = values.index(max(values))
                            personResult = keys[maxIndex]
                            print "I have recognized the person is ----------",personResult
                            self.peopleNameDict = {}
                            access_token = mychat.getTokenIntime()    # wechat alarming
                            message = 'I have recognized the person is ---------- %s' % (personResult,)
